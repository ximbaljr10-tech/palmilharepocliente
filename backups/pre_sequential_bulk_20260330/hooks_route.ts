...
// IMPORTANT: "order.cancelled" refers to LABEL cancellation, NOT order cancellation.
// Label cancellation must NEVER cancel the store order.
function mapSuperfreteStatus(sfStatus: string, event: string): string | null {
  ...
  switch (event) {
    ...
    case "order.cancelled":
      return null // FIXED: Label cancellation must NOT cancel the order
    default:
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    // SuperFrete webhook payload structure:
    // The payload comes as { event, data: { id, status, tracking, ... } }
    // FIXED: Previously was reading body.id (wrong), now reads body.data.id (correct)
    const event = body.event || ""
    const sfData = body.data || {}
    const sfOrderId = sfData.id || sfData.order_id || body.id || body.order?.id || ""
    const sfStatus = sfData.status || body.status || body.order?.status || ""
    const tracking = sfData.tracking || body.tracking || body.order?.tracking || ""
    ...
    // Handle label cancellation specially — NEVER cancel the order
    if (event === "order.cancelled") {
      currentMeta.superfrete_status = "canceled"
      // If order was in preparing (label flow), revert to paid
      if (previousStatus === "preparing") {
        currentMeta.custom_status = "paid"
        currentMeta.superfrete_id = null
        currentMeta.superfrete_tracking = null
        currentMeta.tracking_code = null
        currentMeta.superfrete_print_url = null
        currentMeta.label_cancelled_at = new Date().toISOString()
        statusChanged = true
        console.log(`[WEBHOOK SUPERFRETE] Label cancelled -> order #${medusaOrder.display_id} reverted to PAID (not cancelled)`)
      } else {
        console.log(`[WEBHOOK SUPERFRETE] Label cancelled but order #${medusaOrder.display_id} status kept as ${previousStatus} (not cancelled)`)
      }
    }
    // Update custom_status if we have a mapped status and it's a progression
      ...
      // Only allow forward progression — NEVER cancellation from label events
      if (newIdx > prevIdx) {
