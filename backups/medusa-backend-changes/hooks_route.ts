...
// When a label is cancelled, the order should also show as cancelled.
...
function mapSuperfreteStatus(sfStatus: string, event: string): string | null {
  ...
  switch (event) {
    ...
    case "order.cancelled":
      return "cancelled" // Label cancelled -> order shows as cancelled
    default:
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    // Handle label cancellation — mark order as cancelled
    // Admin can manually revert to 'paid' using the bulk revert feature
    if (event === "order.cancelled") {
      ...
      statusChanged = true
      console.log(`[WEBHOOK SUPERFRETE] Label cancelled -> order #${medusaOrder.display_id} marked as CANCELLED`)
    }
