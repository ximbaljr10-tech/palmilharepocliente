...
async function sendToSuperfrete(order: any): Promise<{ success: boolean; data?: any; error?: string }> {
  ...
  // Medusa stores unit_price in the currency's smallest unit (centavos for BRL)
  // So we divide by 100 to get the value in reais
  const products = (order.items || []).map((item: any) => {
    const rawPrice = Number(item.unit_price) || 0
    // If price looks like it's in centavos (> 100), divide by 100
    // Otherwise assume it's already in reais
    const unitaryValue = rawPrice > 100 ? rawPrice / 100 : rawPrice
    return {
...
export async function PUT(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    if (action === 'batch_revert_to_paid') {
      ...
      console.log(`[BATCH REVERT] Reverting ${order_ids.length} orders to paid...`)
      ...
      for (const oid of order_ids) {
        ...
        // Clear superfrete data and revert to paid
        meta.custom_status = 'paid'
        ...
        delete meta.label_cancelled_at
    ...
    if (action === 'batch_finalize_and_label') {
        ...
        try {
          ...
          if (!sfId) {
            ...
            if (!sfResult.success || !sfResult.data) {
              ...
              // Wait 2 seconds before next order even on failure
          ...
          if (currentMeta.tracking_code) {
            sendOrderEmail("shipped", emailOrder, currentMeta.tracking_code).catch(e => console.error("[BATCH FINALIZE EMAIL]", e.message))
          } else {
            sendOrderEmail("paid", emailOrder).catch(e => console.error("[BATCH FINALIZE EMAIL]", e.message))
          }
      ...
      return res.json({ success: true, total: order_ids.length, succeeded, failed, results })
    }
