      ...
      for (let i = 0; i < ordersWithLabel.length; i += BATCH_SIZE) {
        ...
        const batchPromises = batch.map(async (syncOrder: any) => {
          ...
          try {
            ...
            // IMPORTANT: SuperFrete label cancellation must NEVER cancel the store order.
            // canceled labels only update superfrete_status, NOT custom_status.
            const statusMap: Record<string, string> = {
              released: "preparing", posted: "shipped", delivered: "delivered",
            }
            if (infoData.status === "canceled" || infoData.status === "cancelled") {
              // Label was cancelled — mark it in superfrete metadata but do NOT touch order status
              meta.superfrete_status = "canceled"
              // If order was in preparing (label flow), revert to paid so admin can re-generate
              if (prevStatus === "preparing") {
                ...
                statusChanged = true
                console.log(`[BATCH SYNC] Order #${syncOrder.display_id}: Label cancelled -> order reverted to PAID (not cancelled)`) 
              } else {
                console.log(`[BATCH SYNC] Order #${syncOrder.display_id}: Label cancelled but order status kept as ${prevStatus} (not cancelled)`) 
              }
            } else {
    ...
    if (action === 'sync_superfrete') {
      ...
      try {
        ...
        if (infoData.status === "canceled" || infoData.status === "cancelled") {
          ...
          // If order was in preparing (label flow), revert to paid so admin can re-generate
          if ((prevStatus || "awaiting_payment") === "preparing") {
            ...
            console.log(`[SYNC] Order #${order.display_id}: Label cancelled -> order reverted to PAID (not cancelled)`) 
          } else {
            console.log(`[SYNC] Order #${order.display_id}: Label cancelled but order status kept as ${prevStatus} (not cancelled)`) 
          }
        } else {
          const mappedStatus = statusMap[infoData.status]
          if (mappedStatus) {
