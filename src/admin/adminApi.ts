...
// Shipping dimensions by yards — re-exporta de shippingDefaults.ts (FONTE UNICA)
import {
  getShippingDefaults as _getShippingDefaults,
  extractYardsFromTitle as _extractYardsFromTitle,
...
export function getShippingByYards(yards: number | null, title: string): { height: number; width: number; length: number; weight: number } {
  return _getShippingDefaults(yards, title);
}

// Extract yards from product title
export function extractYards(title: string): number | null {
  return _extractYardsFromTitle(title);
}
export { validateShippingDimensions };
